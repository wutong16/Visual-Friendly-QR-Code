...
class QrPainter(object):
    ...
    def canvas(self):
        ...
        qr_funct = np.zeros((canvas.size,canvas.size))
        for i in range(canvas.size):
            for j in range(canvas.size):
                qr_funct[i][j] = 0 if canvas.points[i][j].fill else 255
        # cv2.imshow("lll",qr_funct)
        # cv2.waitKey(0)
        # qr_funct = cv2.cvtColor(qr_funct, cv2.COLOR_GRAY2BGR)
        basepath = "D:/Python_codes/QR_CODE/pyqart-master/pyqart-master/pyqart/render/"
        name = "qr_funct.jpg"
        # 这里imwrite不是完整路径保存会失败
        cv2.imwrite(basepath+name, qr_funct)
        # print("saved")
        return canvas
